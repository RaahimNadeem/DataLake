...
import aiWhyUsTranslations from '@/translations/Services/AI/aiWhyUS.json';

const AIWhyUS = () => {
  const [current, setCurrent] = useState(0);
  const { language } = useLanguage();
  const currentLang = aiWhyUsTranslations[language as keyof typeof aiWhyUsTranslations];
          ...
          <div className="w-full md:w-1/2 flex flex-col justify-center p-8 md:p-16 text-[#101424]">
            ...
            <div className="flex gap-4 items-center mt-4">
              <button
                ...
                aria-label={currentLang.controls.previousSlide}
              >
                &#8592;
              </button>
              <div className="flex gap-2">
                {slides.map((_, idx) => (
                  <button
                    ...
                    aria-label={`${currentLang.controls.goToSlide} ${idx + 1}`}
                  />
                ))}
              </div>
              <button
                onClick={nextSlide}
                className="w-10 h-10 rounded-full bg-[#eaf1f7] text-[#4a6d8c] flex items-center justify-center text-2xl font-bold hover:bg-[#dbeafe] transition"
                aria-label={currentLang.controls.nextSlide}
              >
                &#8594;
              </button>
            </div>
